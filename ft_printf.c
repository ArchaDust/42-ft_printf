...
/*   Updated: 2021/11/09 14:20:52 by aberneli         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

static int	ft_get_conv(const char *fmt, t_printf *pf, t_conv *conv)
{
	size_t	pos;
	char	*nextconv;
...
static int	ft_parse_fmt(const char *fmt, t_printf *pf, va_list lst)
{
	t_conv	conv;
	t_tfmt	value;
...
int	ft_printf(const char *fmt, ...)
{
	...
	va_end(lst);
	pf.size = write(1, pf.buffer, pf.size);
	free(pf.buffer);
	return (pf.size);
}
